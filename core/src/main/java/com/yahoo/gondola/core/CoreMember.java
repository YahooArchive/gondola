...
public class CoreMember implements Stoppable {
    ...
    public Map<Integer, Peer> peers = new HashMap<>();
    ...
    public CoreMember(Gondola gondola, Shard shard, int memberId, List<Integer> peerIds, boolean isPrimary)
        ...
        peerIds.forEach(id -> peers.put(id, new Peer(gondola, this, id)));
    ...
    */
     ...
     */
    public void forceLeader() throws GondolaException {
        if (!isLeader()) {
    ...
    public int getSavedIndex() throws InterruptedException {
        saveQueue.getLatestWait(savedRid);
     ...
     */
    public void getCommittedLogEntry(CoreCmd ccmd, int index, int timeout)
    ...
    class MainLoop extends Thread {
        ...
        public void mainLoop() throws InterruptedException, GondolaException {
            long waitMs = 0;
    ...
    void handleCommand(CoreCmd ccmd) {
        if (isLeader()) {
    ...
    void become(Role role, int leaderId) throws GondolaException {
        // Clear peers and reset storage state
        peers.forEach((id, p) -> p.reset());
    ...
    public void becomeLeader() throws GondolaException {
     ...
     */
    void sendPrevote() throws GondolaException {
        if (clock.now() >= prevoteTs) {
            // Clear the prevotes before sending prevote
            peers.forEach((id, p) -> p.prevoteGranted = false);
            slaves.forEach(p -> p.prevoteGranted = false);
            sendRequestVoteRequest(true);
        }
    }

    void save(int term, int votedFor) throws GondolaException {
    ...
    void checkHeartbeat() throws GondolaException {
     ...
     */
    void updateCurrentTerm(int term, int fromMemberId) throws GondolaException {
        if (term > currentTerm) {
     ...
     */
    void advanceCommitIndex() throws InterruptedException {
        Map<Integer, Peer> peers = this.peers; // Capture the current state in case peers is modified

        // Get all the match indices
        Arrays.fill(matchIndices, 0);
        final int[] ix = {0};
        peers.forEach((id, p) -> matchIndices[ix[0]++] = p.matchIndex);
    ...
    public boolean showSummary(long waitMs, boolean force) {
        ...
        if (force || now > showSummaryTs) {
            ...
            peers.forEach((id, peer) -> logger.info(peerInfo(peer, false)));
            slaves.forEach(slave -> logger.info(peerInfo(slave, true)));
    ...
    public void sendHeartbeatRequest(Rid rid) {
        ...
        try {
            ...
            peers.forEach((id, p) -> p.send(message, rid.index - 1));
     ...
     */
    public void sendAppendEntryRequest(Message message, int prevLogIndex) {
        // Send command first
        assert message.getType() == Message.TYPE_APPEND_ENTRY_REQ;
        peers.forEach((id, p) -> p.send(message, prevLogIndex));
    ...
    public void sendRequestVoteRequest(boolean isPrevote) throws GondolaException {
        ...
        try {
            ...
            peers.forEach((id, p) -> p.send(message));
        } finally {
            message.release();
        }
    ...
    class MyMessageHandler extends MessageHandler {
        ...
        public boolean appendEntryRequest(Message message, int fromMemberId, int term,
            ...
            Peer peer = peers.get(fromMemberId);
            if (peer == null) {
        ...
        public void requestVoteRequest(Message message, int fromMemberId, int term,
                                       boolean isPrevote, Rid lastRid) throws InterruptedException, GondolaException {
            if (message.tracingInfo != null) {
                logger.info("[{}-{}] recv({}): {}", gondola.getHostId(), memberId, fromMemberId, message.tracingInfo);
            }
        ...
        public void requestVoteReply(Message message, int fromMemberId, int term, boolean isPrevote,
                                     boolean voteGranted) throws GondolaException {
            if (message.tracingInfo != null) {
                logger.info("[{}-{}] recv({}): {}", gondola.getHostId(), memberId, fromMemberId, message.tracingInfo);
            }
            Peer peer = peers.get(fromMemberId);
            ...
            if (term >= currentTerm && isCandidate() && voteGranted) {
                if (isPrevote) {
                    if (prevotesOnly) {
                        ...
                        long prevotes = peers.values().stream().filter(p -> p.prevoteGranted).count() + 1;
                    ...
                    if (!isLeader() && votedFor == memberId) {
                        long votes = peers.values().stream()
                                .filter(p -> p.votedTerm == currentTerm && p.votedFor == memberId)
                                .count() + 1;
    ...
    public void setSlave(int masterId) throws GondolaException, InterruptedException {
        ...
        actionQueue.execute(new Runnable() {
            public void run() {
                try {
                    Map<Integer, Peer> newPeers = new HashMap<>();
                    if (masterId != member.masterId) {
                        // Stop the peers and slaves
                        peers.forEach((id, p) -> p.stop());
                        removeSlaves();
                        ...
                        if (masterId >= 0) {
                            ...
                            masterPeer.start();
                            newPeers.put(masterId, masterPeer);
                            peers = newPeers;
